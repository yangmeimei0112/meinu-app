...
export async function fetchAdminAllData(): Promise<AdminRawDataResult> {
  ...
  const storeGroupsWithStats: GroupOrderAdmin[] = formattedStores.map((store) => {
    ...
    return {
      ...
      status: matchingActiveGroup?.status || 'open',
      announcement: matchingActiveGroup?.announcement ?? null,
      ...
      enable_min_threshold: matchingActiveGroup?.enable_min_threshold ?? false,
      min_threshold_amount: matchingActiveGroup?.min_threshold_amount ?? 0,
      enable_countdown: matchingActiveGroup?.enable_countdown ?? false,
      cutoff_time: matchingActiveGroup?.cutoff_time ?? null,
      enable_budget_limit: matchingActiveGroup?.enable_budget_limit ?? false,
      budget_limit_amount: matchingActiveGroup?.budget_limit_amount ?? 0,
      stores: store,
