...
export function MyOrderHistoryCard({ order, onReorder }: MyOrderHistoryCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#131B2B] rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-xs space-y-3.5">
      {/* 頂部店家與狀態 */}
      <div className="flex items-start justify-between border-b border-slate-100 dark:border-slate-800 pb-3 gap-2 flex-wrap">
        <div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {/* 🌟 訂單進度標籤 */}
          <span
            className={`inline-flex items-center gap-1 text-[11px] font-black px-2.5 py-0.5 rounded-full border shadow-2xs ${progressMeta.badgeBg} ${progressMeta.badgeText} ${progressMeta.badgeBorder}`}
          >
          ...
          {/* 付款狀態標籤 */}
          {order.is_paid ? (
            <span className="inline-flex items-center gap-1 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60 text-[11px] font-extrabold px-2.5 py-0.5 rounded-full shadow-2xs">
              <CheckCircle2 className="w-3 h-3 text-emerald-500" />
              <span>已付款</span>
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60 text-[11px] font-extrabold px-2.5 py-0.5 rounded-full animate-pulse">
