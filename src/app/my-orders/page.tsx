      ...
      <main className="max-w-md mx-auto px-4 pt-3 space-y-4">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-8 text-center border border-slate-100 space-y-3 shadow-xs">
            ...
            <p className="text-sm font-bold text-slate-700">您目前在此裝置上尚無點餐紀錄喔！</p>
            <p className="text-xs text-slate-400">送出訂單後，可隨時在此追蹤付款狀態與一鍵再點一次。</p>
            <Link
              href="/"
          ...
          <div className="space-y-3.5">
            {orders.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-white rounded-3xl p-5 border border-slate-100 shadow-xs space-y-3.5"
                >
                  {/* 頂部店家與狀態 */}
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    ...
                    <div>
                      {order.is_paid ? (
                        <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11px] font-extrabold px-3 py-1 rounded-full shadow-2xs">
                          <span>✅</span>
                          <span>已付款</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200 text-[11px] font-extrabold px-3 py-1 rounded-full animate-pulse">
                          <span>⏳</span>
                          <span>待對帳</span>
                        </span>
                      )}
                  ...
                  <div className="divide-y divide-slate-100">
                    {order.order_items.map((item) => (
                      <div key={item.id} className="py-2 space-y-0.5">
                        ...
                        {item.custom_notes && (
                          <p className="text-[11px] text-slate-400">{item.custom_notes}</p>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                    <span className="text-slate-500">
                      付款方式：<span className="font-bold text-slate-700">{order.payment_method_name}</span>
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button
                      ...
                      className="bg-sky-50 hover:bg-sky-100 text-sky-700 font-bold text-xs py-2.5 rounded-2xl border border-sky-100 transition active:scale-95 flex items-center justify-center gap-1"
                    >
                      <span>🔄 一鍵再點一次</span>
                    </button>
                    ...
                    <Link
                      ...
                      className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs py-2.5 rounded-2xl text-center shadow-xs transition active:scale-95 flex items-center justify-center gap-1"
                    >
                      <span>查看詳細 ➔</span>
                    </Link>
                  </div>
